...
class DatabasesConfig(BaseConfig):
    ...
    def read_section(self, parser, section):
        ...
        for name, sub_config in self.databases.items():
            ...
            elif sub_section.endswith(":ro") and parser.has_section(
                sub_section.replace(":ro", "")
            ):
    ...
    def read_env(self, prefix):
        ...
        read_env_item(
            self,
            "use_auto_commit",
            prefix + "DATABASE_AUTO_COMMIT",
            convert=str_to_bool,
        )
        read_env_item(
            self,
            "use_two_phase_commit",
            prefix + "DATABASE_TWO_PHASE_COMMIT",
            convert=str_to_bool,
        )
        for name, sub_config in self.databases.items():
...
class DatabaseConfig(BaseConfig):
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.pool_recycle == other.pool_recycle
            and self.pool_pre_ping == other.pool_pre_ping
    ...
    def read_env(self, prefix):
        ...
        read_env_item(
            self, "pool_pre_ping", prefix + "POOL_PRE_PING", convert=str_to_bool
        )
        ...
        for name, value in os.environ.items():
            if name.startswith(level_field_prefix):
                if name == level_field_prefix:
                    ...
                    logger_name = (
                        name.replace(level_field_prefix + "_", "")
                        .lower()
                        .replace("__", ".")
                    )
...
class WebSiteConfig(BaseConfig):
    ...
    def read_section(self, parser, section):
        ...
        read_config_secret_str_option(
            parser, section, self, "mb_oauth_client_id", "mb_oauth_client_id"
        )
        read_config_secret_str_option(
            parser, section, self, "mb_oauth_client_secret", "mb_oauth_client_secret"
        )
        read_config_secret_str_option(
            parser, section, self, "google_oauth_client_id", "google_oauth_client_id"
        )
        read_config_secret_str_option(
            parser,
            section,
        ...
        if parser.has_option(section, "search_return_metadata"):
            self.search_return_metadata = parser.getboolean(
                section, "search_return_metadata"
            )
    ...
    def read_env(self, prefix):
        ...
        read_env_item(
            self, "google_oauth_client_secret", prefix + "GOOGLE_OAUTH_CLIENT_SECRET"
        )
...
class uWSGIConfig(BaseConfig):
    ...
    def read_env(self, prefix):
        ...
        read_env_item(
            self, "http_connect_timeout", prefix + "UWSGI_CONNECT_TIMEOUT", convert=int
        )
        read_env_item(self, "workers", prefix + "UWSGI_WORKERS", convert=int)
        read_env_item(
            self, "post_buffering", prefix + "UWSGI_POST_BUFFERING", convert=int
        )
        read_env_item(self, "buffer_size", prefix + "UWSGI_BUFFER_SIZE", convert=int)
        read_env_item(
            self, "offload_threads", prefix + "UWSGI_OFFLOAD_THREADS", convert=int
        )
...
class ReplicationConfig(BaseConfig):
    ...
    def read_section(self, parser, section):
        ...
        if parser.has_option(section, "import_acoustid_musicbrainz"):
            self.import_acoustid_musicbrainz = parser.get(
                section, "import_acoustid_musicbrainz"
            )
...
class ClusterConfig(BaseConfig):
    ...
    def read_section(self, parser, section):
        ...
        read_config_str_option(
            parser, section, self, "base_master_url", "base_master_url"
        )
        read_config_secret_str_option(parser, section, self, "secret", "secret")
    ...
    def read_section(self, parser, section):
        ...
        for name in parser.options(section):
            ...
            if name.startswith("ip."):
                ...
                self.applications[int(name.split(".", 1)[1])] = parser.getfloat(
                    section, name
                )
...
class SentryConfig(BaseConfig):
    ...
    def read_section(self, parser, section):
        if parser.has_option(section, "dsn"):
